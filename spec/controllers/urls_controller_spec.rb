...
RSpec.describe "Urls", type: :request do
  describe "POST /encode" do
    ...
    it "creates a new short URL" do
      post encode_path, params: { url: original_url }
    ...
    it "returns error if URL is Invalid" do
      post encode_path, params: { url: "google.com" }

      json = JSON.parse(response.body)

      expect(response).to have_http_status(:unprocessable_content)
      expect(json["error"]).to eq("url attribute is not a Valid URL")
    end
  end
  ...
  describe "POST /decode" do
    ...
    it "returns error if URL is Invalid" do
      post decode_path, params: { url: "google.com" }
